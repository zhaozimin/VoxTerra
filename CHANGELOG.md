...
- **黑底 logo + 版本同行**：摘要卡改用正在使用的黑底 App 图标（裁自 `VoiceLog_iconmaster.png` 满铺），版本号紧随「言壤」之后；移除退出按钮上方的独立版本行，更清晰。
- **链接转中性**：作者三链接（主页/Obsidian/GitHub）由品牌彩色改为中性矢量图标，遵「单一绿强调」铁律。
- **定版**：作为成熟稳定测试版 1.0 交付；机器名/bundle id（com.zhaozimin.voicelog）/数据目录一律不动。Windows 版本号独立演进，不随此次升级。
...
- **静默启动**：开启后启动不显示窗口、隐藏 Dock，仅菜单栏在后台记录；需要时从菜单栏「打开主窗口」一键回前台。
- **设计**：两态用激活策略 .accessory/.regular 一刀切、无中间态；「打开主窗口」回前台逻辑幂等，不改变非静默用户现有行为。纯前端 App 偏好（不入引擎 config），新增 Startup.swift 统管。
...
- **取消注册频闪修复**：在朗读窗点「取消」时，引擎要 1~2 拍才停，陈旧的 `enrolling=true` 会把浮层拉回 →
  关了又开。Swift 加 `enrollCancelling` 守护:取消后在引擎确认停下前一律无视陈旧 true。
- **引擎注册单飞(根因)**：headless 下 `cancel→立刻 enroll` 会被合批消费，旧采集线程没退、新线程又起 →
  两线程争写状态(瞬时 false 闪)且帧交错**污染声纹**。给注册加**代际令牌**(同下载 `model_dl_gen`)，
  被取代的旧线程静默退出、绝不碰共享状态/不写声纹。
- **重注册残值闪修复**：进度只在引擎真正在录该轮时取真值，否则归零——消除重注册开头误显「0秒/采集完成」。
注册稿子定稿:把"只录声音不录内容"与"读完可再读"两句说明全并入朗读正文，稿子下方不再另设提示。

## v0.9.11 — 2026-06-22
注册稿子定稿:把「只采集声音、与读什么无关」与「读完可再读一遍」两句说明全并入朗读正文，稿子框下方不再另设提示。

## v0.9.10 — 2026-06-22
精修声纹注册稿子。
- 把「它只采集你的声音，与你读的内容无关」并入**朗读稿子正文**(读出来也是说明，一举两得)。
- 删去稿子里的天气/阳光/鸟鸣等无关填充；稿子下方仅保留「读完倒计时还没结束？从头再读一遍就行。」。

## v0.9.9 — 2026-06-22
注册稿子提示 + 全局可点反馈 + 开屏导航颤抖根治。
- **声纹注册朗读窗加末尾提示**：「读完了倒计时还没结束？从头再读一遍就行——它只采集你的声音，与你读的内容无关。」
- **所有可点元素加悬停反馈**：新增 `clickable()` 修饰器(Tahoe 原生 `pointerStyle(.link)` 手型指针)，
  统一加到模型下载/使用/删除/取消、设置各标签与按钮、语言时区下拉、首页暂停、关于链接、托盘、日历等所有可点处；
  导航/设置标签的未选中悬停态补半透明卡底高亮。鼠标移上去即知"可点"。
- **开屏导航颤抖根治**：旧法靠"测量→偏移修正"在开屏布局逐拍落定时产生可见跳动。改为**收敛前隐藏**——
  位置连续两拍不变(布局 settle)才淡入，整个抖动过程对用户不可见；另设 2.5s 兜底强制显示，杜绝永久隐身。

## v0.9.8 — 2026-06-22
声纹注册补齐两段式流程 + 设置标签悬停高亮。
- **声纹注册改为两段式、有稿可读**：点「注册/重新注册」先弹**确认须知**(「确定」开始、「取消」关闭，两键真实生效，
  文案"点击确认后，照着稿子读就可以了。")→ 确定后进**朗读窗**：显示一段稿子 + 倒计时(读出声才倒数)+ 进度，
  采够约 20 秒有效语音自动完成，全程可取消。此前朗读窗无内容、不知读什么。
- **设置页标签悬停高亮**：语言时区/模型下载/保存位置/关键词/参数/配置文件等标签，鼠标移上去显半透明卡底，
  明确"可点"提示(此前仅文字微变色，几乎看不出)。

## v0.9.7 — 2026-06-22
0.9.6 实测回归修复(全 SwiftUI 侧):导航微移 + 模型卡抖动 + 声纹注册可用。
- **导航胶囊点击不再轻微漂移**：居中测量的死区从 0.5px 放宽到 2px，滤掉 Retina/CJK 子像素舍入造成的
  ≤2px 抖动(初次居中的大位移照常生效)；并延长落定补测到 3s,覆盖窗口/工具栏/状态丸的迟到布局。
- **模型卡抖动修复**：① 切档下载不再先闪「100%」再归零——守护期不再误用全局进度残值；
  ② 切换「使用中」不再来回横跳——modelName 加归属守护(引擎写回 config.model 前守住乐观值)。
- **「注册声音」真正可用**：补全注册浮层(朗读提示 + 有效语音进度 + 取消)与 enrolling/进度回显;
  此前只默默启动引擎采集、无任何提示与进度,形同无效。

## v0.9.6 — 2026-06-22
原生 SwiftUI 完整版打磨：下载断点续传重写 + 消除两处闪烁 + 设置保存。
- **模型下载改成「自带重试的断点续传」**：为慢且频繁中断的网络而生——点一次就走开,它自己扛。断后从 `.part`
  精确字节带 Range 自动续(不从 0 来)、有进展清失速且每次断后至少歇 1s 杜绝热循环、连续多次零进展或累计下载
  超 4G 即放弃绝不挂死;权威总大小取 Content-Range(认 206 分片重组);`.part` 移到模型同盘(不被系统清、落地
  不跨盘拷贝)。新增**取消**(下载卡 ✕,已下进度保留下次续)、删除顺带清 `.part`、下载失败/中断在界面给明确反馈。
- **消除两处「乐观写被轮询闪回」的闪烁**：切换语言/时区不再闪、点下载/续传卡片不再闪回「下载」——
  统一用「归属保护」(引擎确认前守住本地值)。从 64% 续传不再先闪 0%。
- **语言/时区/参数/关键词真正保存并回显**：点选即写 `config.yaml`,重开设置值还在。
- 设置默认打开「语言和时区」;首页今日记录倒序(最新在上),Markdown 仍按时间正序。
...
### 1.0 打磨：首启 / 兜底 / 诚实（消除「沉默失败」与「文档画饼」）
- **首次启动欢迎页**：明确告知「正在后台聆听麦克风、音频从不存盘」(consent 时刻) + 三步上手 + 笔记位置；只弹一次。
- **麦克风被拒不再静默**：检测授权状态，被拒时醒目提示 + 一键跳「系统设置 → 隐私 → 麦克风」。
- **Apple 芯片预检**：Intel Mac 上友好提示并退出，不再无界面原生崩溃。
- **err.log >1MB 自动轮转**(mac/win) + 新增菜单「打开日志文件夹」，出问题一键可达。
- **写入失败 / 掉到内置备用盘 → 一次性系统通知**，数据问题不再悄无声息。
- **文档诚实化**：README 的「夜间整理」明确标注为**手动配方(非内置开关)**，与代码一致。
