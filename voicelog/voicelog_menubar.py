...
# 内置注册朗读稿：几句「平常会说的话」即可。ECAPA 声纹是 text-independent——只认音色不认内容，
# 真正决定质量的是「足够的自然语音时长」(已由质量驱动保证)，不必刻意把数字/字母/单词都读到。
# 所以选最自然好读的日常口语 + 一句带数字的童谣(顺带覆盖数字,但不刻意)，读着不累、像平时聊天。
DEFAULT_ENROLL_SCRIPT = """用平常说话的语气，自然地读下面几句就好——
系统采的是你的「音色」，不是内容，读错了也完全没关系。

你好，我现在在录我自己的声音。
今天天气还不错，我打算先忙一会儿，晚点出去走走。
一二三四五，上山打老虎；六七八九十，开心每一天。
周末想睡到自然醒，再约朋友吃个饭、看场电影。
说话不用快，就像平时跟人聊天一样放松。

如果还没结束，就把上面几句再念一遍，直到提示「采集完成」。"""
...
# ---------------- 纠错表的 文本 <-> 字典 <-> config 三态同步（供关键词窗口用） ----------------
def rules_to_text(rules: dict) -> str:
    """纠错表 → 逐行「错误 = 正确」文本，喂给关键词窗口编辑。"""
    return "\n".join(f"{k} = {v}" for k, v in rules.items())


def text_to_rules(text: str) -> dict:
    """逐行文本 → 纠错表：每行 `错误=正确`，忽略空行与 # 注释，以首个 = 分隔。"""
    rules = {}
...
def write_replace(rules: dict) -> None:
    """更新纠错表：① 立即生效(改全局 REPLACE，apply_replace 下句即用) ② 写回 config.yaml 的 replace 块。"""
    REPLACE.clear()
    ...
    try:
        cfg = BASE / "config.yaml"
        text = cfg.read_text(encoding="utf-8")
        pat = re.compile(r"(?ms)^replace:.*?(?=^[^\s#]|\Z)")  # replace: 到下个顶层键或文件尾
        new = pat.sub(lambda m: block, text) if pat.search(text) else text.rstrip() + "\n\n" + block
        cfg.write_text(new, encoding="utf-8")
    except Exception:
        append_err("write_replace: " + traceback.format_exc().splitlines()[-1])
...
class VoiceLogApp(rumps.App):
    def __init__(self):
        ...
        self.menu = [
            ...
            self.tz_menu,
            self.vault_item,
            rumps.MenuItem("关键词纠错…", callback=self.do_replace),
    ...
    def do_replace(self, _):
        try:
            ...
            self._replace_win = ReplaceWindow(rules_to_text(REPLACE), on_save=self._save_replace)
        except Exception:
            append_err("ReplaceWindow: " + traceback.format_exc())
            rumps.alert("打不开关键词窗口", "请查看 logs/err.log。")

    def _save_replace(self, text):
        rules = text_to_rules(text)
        write_replace(rules)
        win = getattr(self, "_replace_win", None)
        if win:
            win.close()
        self._replace_win = None
        rumps.alert("关键词纠错", f"已保存 {len(rules)} 条纠错规则，立即生效。")
