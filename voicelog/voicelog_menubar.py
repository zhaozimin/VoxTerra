...
# 内置注册朗读稿：让用户「亲口念出这款应用怎么工作、怎么用」——一举三得：
#   ① 念出来即提供声纹样本(ECAPA text-independent，内容随意，关键是足够自然语音时长，质量驱动已保证)；
#   ② 读一遍就把隐私承诺(本地/不上传/即用即弃)自己说一遍，比看说明记得牢；
#   ③ 顺带把使用方式与细节(常驻菜单栏、后台免操作、按日期成笔记、可暂停、静音不乱编)也读进脑子里。
# 第一人称、平常口语，念着自然；字数足够一遍读满 ~20 秒。可在 config 的 enroll_script 自定义。
DEFAULT_ENROLL_SCRIPT = """你好，我正在录入我自己的声音。
...
# 注册「须知页」文案：第一屏只做简短引导——告诉用户「接下来照着念」，且念的内容正是产品说明，一举两得。
# 详细的隐私承诺放进 DEFAULT_ENROLL_SCRIPT 让用户亲口读出来(读比看记得牢)。可在 config 的 enroll_intro 自定义。
DEFAULT_ENROLL_INTRO = """接下来，请照着屏幕上的几句话，用平常聊天的语气念出来。
...
def current_prompt():
    """实际喂给 whisper 的 initial_prompt = 基础提示 + 识别词库(TERMS)。词库从源头偏置识别，零误伤。"""
    base = INITIAL_PROMPT or ""
    if TERMS:
        base = (base + " 可能出现的词：" + "、".join(TERMS) + "。").strip()
    return base or None
    ...
    def _transcribe(self, utt: np.ndarray):
        try:
            result = mlx_whisper.transcribe(
                ...
                initial_prompt=current_prompt(),    # 基础提示 + 识别词库；简体偏置、压繁体
...
class VoiceLogApp(rumps.App):
    def __init__(self):
        ...
        self.menu = [
            ...
            self.tz_menu,
            self.vault_item,
            rumps.MenuItem("关键词管理…", callback=self.do_replace),
    ...
    def do_replace(self, _):
        ...
        rumps.alert("关键词管理",
                    f"已保存：{len(rules)} 条精确纠错 + {len(terms)} 个识别词，立即生效。")
