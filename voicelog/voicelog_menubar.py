...
from model_fetch import model_ready, model_status_key, download_model, get_model_url, MAC_MODEL_URL, model_hint
import update_check
import auto_update
import i18n
...
# ---------------- Phase 2 文件桥接：SwiftUI 全窗口前端 ↔ 本引擎 ----------------
# 桥接文件钉死在固定位置(不随 源码态 DATA=源码目录 / 打包态 飘移)：Swift 端默认读这里；
# state.json 的 paths 段自描述真实 config/vault/data 位置，消除两态歧义。对菜单栏零影响。
BRIDGE = Path.home() / "Library" / "Application Support" / "VoiceLog"
BRIDGE.mkdir(parents=True, exist_ok=True)
STATE_PATH = BRIDGE / "state.json"           # 引擎每拍快照写出，Swift 读
COMMANDS_PATH = BRIDGE / "commands.json"     # Swift 写命令，引擎 tick 消费(Phase 2b)
...
# 声纹验证最短时长：短于此(秒)的句子跳过声纹门——ECAPA 对 <~1.2s 的短音频验不准，
# 否则会把机主的「早上好」这类短句误判成他人丢掉。近场/能量门仍挡远场短噪声。
SPEAKER_MIN_VERIFY_SEC = float(CFG.get("speaker_min_verify_sec", 1.2))
...
# ---------------- App 形态：lite(菜单栏) vs full(Dock 全窗口产品) ----------------
# 同一套引擎，两层外壳：full 多挂一个主窗口并常驻 Dock，菜单栏照旧。env 优先于 config，默认 lite。
FLAVOR = (os.environ.get("VOICELOG_FLAVOR") or CFG.get("app_flavor") or "lite").strip().lower()
FULL_APP = FLAVOR == "full"
...
# ---------------- 设置窗口：写回 config + 即时生效 ----------------
def set_config_values(vals: dict) -> None:
    """逐键替换 config.yaml 里那行(不存在则追加),保留其余注释。"""
    try:
        ...
        append_err("写回设置失败：" + traceback.format_exc().splitlines()[-1])


def apply_settings(app, vals: dict) -> None:
    """即时生效:每句现读的门控立刻吃新全局;SpeakerGate 阈值同步;VAD/断句触发干净重开流。"""
    global VAD_THRESHOLD, MIN_SPEECH_MS, MIN_SILENCE_MS, MIN_RMS_DBFS
    global SPEAKER_THRESHOLD, SPEAKER_MIN_VERIFY_SEC, MAX_UTT_SEC
    VAD_THRESHOLD = float(vals.get("vad_threshold", VAD_THRESHOLD))
    MIN_SPEECH_MS = int(vals.get("min_speech_ms", MIN_SPEECH_MS))
    MIN_SILENCE_MS = int(vals.get("min_silence_ms", MIN_SILENCE_MS))
    MIN_RMS_DBFS = float(vals.get("min_rms_dbfs", MIN_RMS_DBFS))
    SPEAKER_THRESHOLD = float(vals.get("speaker_threshold", SPEAKER_THRESHOLD))
    SPEAKER_MIN_VERIFY_SEC = float(vals.get("speaker_min_verify_sec", SPEAKER_MIN_VERIFY_SEC))
    MAX_UTT_SEC = float(vals.get("max_utterance_sec", MAX_UTT_SEC))
    try:
        app.rec.speaker.threshold = SPEAKER_THRESHOLD   # 已构造的 SpeakerGate 用自身 threshold,需同步
        app.rec._reload = True                          # VAD/断句烤进 VADIterator → 触发重开流吃新值
    except Exception:
        pass


# ---------------- 四档模型：按 id 下载 / 切换 / 删除（菜单栏与 headless 共用，写进度入 state）----------------
def _model_dir(model_id: str) -> Path:
    return DATA / "models" / f"whisper-mlx-{model_id}"


def download_model_by_id(state: dict, model_id: str) -> None:
    """后台下载某档模型,进度写 state(model_dl/model_dl_id/model_pct/model_result/model_result_id)。
    下载器自带重试断点续传(慢/抖网络点一次磨到下满);state['model_cancel']=True 可中途喊停。幂等。
    代际守护:每次下载领一个递增 gen,_run 收尾只在「我仍是当前下载」时落终态——
    防『取消后立刻重点』时旧线程的收尾写回踩坏新下载的状态。"""
    if model_id not in ("tiny", "base", "small", "turbo"):
        append_err(f"download_model_by_id: invalid {model_id!r}"); return
    dest = _model_dir(model_id)
    if model_ready(dest) or state.get("model_dl"):
        return
    gen = state.get("model_dl_gen", 0) + 1
    state["model_dl_gen"] = gen
    state["model_dl"] = True
    state["model_dl_id"] = model_id
    # 续传:.part 已有进度则保留上次百分比,避免从 64% 闪回 0%;全新下载才归零
    state["model_pct"] = 0 if not (dest.parent / (dest.name + ".part.zip")).exists() else state.get("model_pct", 0)
    state["model_cancel"] = False                     # 新下载:清掉上次遗留的取消旗标
    state["model_result"] = None                      # 清掉上次的失败/成功标记,免得卡片显示陈旧结果
    state["model_result_id"] = None

    def _run():
        status = {}
        try:
            download_model(get_model_url(model_id, "mac"),
                           dest,
                           lambda f: state.__setitem__("model_pct", round(f * 100)),
                           should_cancel=lambda: bool(state.get("model_cancel")),
                           status=status)
        finally:                                       # 即便 finalize 抛异常也要复位,否则 model_dl 泄漏=后续全堵
            if state.get("model_dl_gen") == gen:       # 仅当我仍是当前下载,才落终态(代际守护)
                state["model_dl"] = False
                state["model_dl_id"] = None
                state["model_cancel"] = False
                state["model_result"] = status.get("state", "fail")  # 判定来自下载器自身,非事后读旗标
                state["model_result_id"] = model_id
    threading.Thread(target=_run, daemon=True).start()


def use_model_by_id(model_id: str) -> bool:
    """切到某已下载档位:写 config.model 为其绝对路径 + 更新全局(下次转写/重开流即用)。"""
    global MODEL, MANAGED_MODEL
    dest = _model_dir(model_id)
    if not model_ready(dest):
        return False
    set_config_str("model", str(dest))
    MODEL = str(dest)
    MANAGED_MODEL = False
    return True


def delete_model_by_id(state: dict, model_id: str) -> None:
    """删除某档(使用中的不删);顺带清掉同名 .part.zip 残片,回收磁盘。
    正在下载的同档拒删——否则会与下载线程争同一 .part(删它写它),既毁下载又泄漏 model_dl。"""
    if state.get("model_dl") and state.get("model_dl_id") == model_id:
        return                                        # 该档正在下载:先取消再删,这里直接拒
    import shutil
    dest = _model_dir(model_id)
    if dest.exists() and str(dest) != str(MODEL):
        shutil.rmtree(dest, ignore_errors=True)
    part = dest.parent / (dest.name + ".part.zip")    # 未下完的残片也一并删,免得占着 1.5G
    part.unlink(missing_ok=True)


def _config_snapshot() -> dict:
    """从 config.yaml 现读出供 UI 回显的设置值(语言/时区/参数/关键词)。每拍读一次,反映已保存值。"""
    try:
    ...
    kw = "\n".join([f"{k} = {v}" for k, v in rep.items()] + [str(t) for t in terms])
    return {
        "ui_language": c.get("ui_language", "") or "",
        "primary_language": c.get("primary_language", "") or "",
        "secondary_language": c.get("secondary_language", "") or "",
        "timezone": c.get("timezone", "") or "",
        "vad_threshold": float(c.get("vad_threshold", 0.6)),
        "min_speech_ms": int(c.get("min_speech_ms", 300)),
        "min_silence_ms": int(c.get("min_silence_ms", 700)),
        "min_rms_dbfs": float(c.get("min_rms_dbfs", -45.0)),
        "speaker_threshold": float(c.get("speaker_threshold", 0.35)),
        "speaker_min_verify_sec": float(c.get("speaker_min_verify_sec", 1.2)),
        "max_utterance_sec": float(c.get("max_utterance_sec", 30)),
        "keywords": kw,
    }
...
class Recorder(threading.Thread):
    def __init__(self, state: dict):
        ...
        self._reload = False  # 设置窗口保存 VAD/断句类参数后置 True → 干净重开流,用新值重建 VADIterator
    ...
    def _stream_loop(self):
        ...
        with sd.InputStream(samplerate=SR, channels=1, dtype="float32",
            ...
            while True:
                if self._reload:                 # 设置已改 VAD/断句 → 干净返回,run() 立刻用新值重建 VADIterator
                    self._reload = False
                    return
                try:
    ...
    def _accept(self, utt: np.ndarray) -> bool:
        ...
        # ③ 声纹门：只验「够长、验得准」的句子。太短(< SPEAKER_MIN_VERIFY_SEC)直接放行——
        #    ECAPA 对不到 ~1.2s 的短音频本就验不准，硬验会把机主的「早上好」误丢(近场/能量门仍挡远场)。
    ...
    def enroll(self):
        ...
        **单飞(代际令牌)**:headless 下 cancel→立刻 enroll 会被合批消费,旧线程尚在 sd.sleep 没退、新线程又起→
        两线程争写 enrolling/共享缓冲 → 瞬时 false 闪 + 帧交错污染声纹。故每轮领一个 gen,被取代的旧线程一律
        静默退出,绝不碰共享状态/不写声纹(仿下载 model_dl_gen 的同一套纪律)。"""
        self._enroll_cancel = False
        gen = getattr(self, "_enroll_gen", 0) + 1
        self._enroll_gen = gen
        ...
        def _run():
            ...
            while (not self._enroll_cancel and self._enroll_gen == gen
                   and voiced_sec < ENROLL_VOICED_SEC and elapsed < ENROLL_MAX_SEC):
                sd.sleep(200)
                elapsed += 0.2
            ...
            if self._enroll_gen != gen:             # 已被新一轮注册取代:静默退出,绝不碰共享状态/声纹
                return
            self._enroll = None
...
class VoiceLogApp(rumps.App):
    def __init__(self):
        ...
        self.settings_item = rumps.MenuItem(i18n.t("settings_menu"), callback=self.do_settings)  # 滑块调参 GUI
        self.config_item = rumps.MenuItem(i18n.t("open_config"), callback=self.open_config)  # 高级:整份 config
        ...
        self.main_win = None  # full 形态下的 Dock 主窗口(下方构建)；lite 恒为 None
        self.open_item = rumps.MenuItem(i18n.t("win_open"), callback=self.open_main) if FULL_APP else None
        self.menu = ([self.open_item, None] if FULL_APP else []) + [
            self.count_item,
            self.toggle_item,
            None,  # 分隔线
        ...
        # full 形态：常驻 Dock + 构建主窗口(首页/历史/设置/关于)，与菜单栏并存
        if FULL_APP:
            import ui_common
            ui_common.floor_regular()
            try:
                from main_window import MainWindow
                self.main_win = MainWindow(self)
            except Exception:
                append_err("MainWindow init: " + traceback.format_exc())

    def open_main(self, _=None):
    ...
    def _apply_language(self):
        ...
        self.settings_item.title = i18n.t("settings_menu")
        self.config_item.title = i18n.t("open_config")
    ...
    def _write_state(self):
        """Phase 2 桥接：每拍把运行状态原子快照到 BRIDGE/state.json，供 SwiftUI 全窗口前端读取。
        纯读 self.state/全局，绝不改业务；写 tmp 再 os.replace，读端永不读到半截。"""
        try:
            st = self.state
            ts = now()
            snap = {
                "schema": 1,
                "heartbeat": time.time(),       # Swift 判 now-heartbeat>6s → 引擎停摆，UI 降级
                "pid": os.getpid(),
                "version": VERSION,
                "muted": self.rec.muted,
                "live": bool(st.get("live")),
                "count": st.get("count", 0),
                "dropped": st.get("dropped", 0),
                "last": st.get("last", ""),
                "sink": st.get("sink", "vault"),
                "err": st.get("err", ""),
                "today_key": f"{ts:%Y-%m-%d}",   # 引擎时区的今天，Swift 不自己算
                "today_file": str(VAULT / f"{ts:%Y-%m-%d}.md"),
                "model": {
                    "ready": model_ready(MODEL),
                    "name": self._model_name(),
                    "managed": MANAGED_MODEL,
                    "downloading": bool(st.get("model_dl")),
                    "pct": st.get("model_pct", 0),
                    "dl_id": st.get("model_dl_id"),
                    "result": st.get("model_result"),        # ok/cancelled/fail/None → SwiftUI 卡片显失败反馈
                    "result_id": st.get("model_result_id"),  # 哪一档的结果(供卡片对位)
                },
                "speaker": {
                    "enrolled": bool(st.get("enrolled")),
                    "gate_on": self.rec.speaker_on,
                    "last_score": st.get("last_score"),
                    "threshold": SPEAKER_THRESHOLD,
                    "enrolling": bool(st.get("enrolling")),
                    "enroll_progress": st.get("enroll_progress", 0.0),
                    "enroll_voiced": st.get("enroll_voiced", 0.0),
                    "enroll_quality": st.get("enroll_quality"),
                },
                "update": {
                    "checked": bool(st.get("update_checked")),
                    "latest": st.get("update_latest"),
                },
                "paths": {
                    "data_dir": str(DATA),
                    "vault": str(VAULT),
            ...
            append_err("_write_state: " + traceback.format_exc().splitlines()[-1])

    def _process_commands(self):
        """Phase 2 桥接：消费 SwiftUI 写入的 BRIDGE/commands.json。读到即删(至多执行一次)，
        逐条 try——单条失败只记日志，绝不拖垮 tick；异步活(下载/注册/查更新)一律开线程。"""
        if not COMMANDS_PATH.exists():
            return
        try:
            raw = COMMANDS_PATH.read_text(encoding="utf-8")
            COMMANDS_PATH.unlink()                 # 读到即删
        except Exception:
            append_err("_process_commands read: " + traceback.format_exc().splitlines()[-1])
            try: COMMANDS_PATH.unlink()
            except Exception: pass
            return
        try:
            cmds = json.loads(raw) or []
        except Exception:
            return
        ...
        for c in cmds:
            try:
                self._dispatch((c.get("cmd") or "").strip(), c.get("args") or {})
            except Exception:
                append_err(f"cmd {c!r}: " + traceback.format_exc().splitlines()[-1])

    def _dispatch(self, cmd, a):
        """把一条命令落到现成的引擎函数上（不复制业务）。"""
        if cmd in ("pause", "resume", "toggle_mute"):
            self.rec.muted = {"pause": True, "resume": False}.get(cmd, not self.rec.muted)
            self.toggle_item.title = i18n.t("resume") if self.rec.muted else i18n.t("pause")
        elif cmd == "speaker_gate":
            self.rec.speaker_on = bool(a.get("on"))
            set_config_flag("speaker_gate", self.rec.speaker_on)
            self.spk_item.title = self._spk_title()
        elif cmd == "enroll":
            self.do_enroll(None)
        elif cmd == "cancel_enroll":
            self.rec.cancel_enroll()
        elif cmd == "set_vault":
            if a.get("path"):
                set_vault(a["path"])
            ...
            set_timezone(a.get("tz", "") or "")
        elif cmd == "set_language":
            m = {"follow": "", "none": ""}
            if "ui" in a:
                v = m.get(a["ui"], a["ui"]); set_config_str("ui_language", v); i18n.set_language(v)
            if "primary" in a:
                v = m.get(a["primary"], a["primary"]); set_config_str("primary_language", v); i18n.set_primary(v)
            if "secondary" in a:
                v = m.get(a["secondary"], a["secondary"]); set_config_str("secondary_language", v); i18n.set_secondary(v)
        elif cmd == "save_keywords":
            res = parse_corrections(a.get("text", ""))
            if isinstance(res, tuple) and len(res) == 2:
                write_corrections(res[0], res[1])
        elif cmd == "set_params":
            keys = ("vad_threshold", "min_speech_ms", "min_silence_ms", "min_rms_dbfs",
                    "speaker_threshold", "speaker_min_verify_sec", "max_utterance_sec")
            vals = {k: a[k] for k in keys if k in a}
            if vals:
                set_config_values(vals)
                apply_settings(self, vals)
        elif cmd == "model_download":
            download_model_by_id(self.state, a.get("id", "turbo"))
        elif cmd == "model_cancel":
            self.state["model_cancel"] = True       # 喊停:下载线程下一拍自停,留 .part 供下次续
        elif cmd == "model_use":
            use_model_by_id(a.get("id", "turbo"))
        elif cmd == "model_delete":
            delete_model_by_id(self.state, a.get("id", ""))
        elif cmd == "check_update":
            threading.Thread(target=self._check_update, daemon=True).start()
        elif cmd == "open_config":
            self.open_config(None)
        else:
            append_err(f"unknown cmd: {cmd!r}")
    ...
    def tick(self, _):
        ...
        # full 形态：首帧把主窗口端到前台(在 run 循环里 show 比 __init__ 更稳)，此后每拍刷新首页
        if self.main_win is not None:
            if not getattr(self, "_main_shown", False):
                self._main_shown = True
                self.main_win.show()
            self.main_win.refresh()

        self._process_commands()   # Phase 2 桥接：先消费 SwiftUI 命令(当拍生效)
        self._write_state()        # 再快照运行状态(把命令效果一并反映给前端)
    ...
    def _download_model(self):
        ...
        status = {}
        try:
            download_model(MAC_MODEL_URL, MODEL,
                           lambda f: self.state.__setitem__("model_pct", round(f * 100)),
                           should_cancel=lambda: bool(self.state.get("model_cancel")),
                           status=status)
        finally:                                   # finalize 抛异常也复位,否则 model_dl 泄漏=菜单卡在"下载中"
            self.state["model_dl"] = False
            self.state["model_cancel"] = False
            self.state["model_result"] = status.get("state", "fail")
    ...
    def open_config(self, _=None):
        # 在默认文本编辑器里打开配置(整份 config,含术语/纠错等设置窗外的项);改完重启生效
        subprocess.run(["open", "-t", str(CONFIG_PATH)])

    def do_settings(self, _):
        """滑块参数设置窗口：保存即写回 config.yaml 并即时生效。"""
        if getattr(self, "_settings_open", False):
            return
        cur = {"vad_threshold": VAD_THRESHOLD, "min_speech_ms": MIN_SPEECH_MS,
               "min_silence_ms": MIN_SILENCE_MS, "min_rms_dbfs": MIN_RMS_DBFS,
               "speaker_threshold": SPEAKER_THRESHOLD,
               "speaker_min_verify_sec": SPEAKER_MIN_VERIFY_SEC, "max_utterance_sec": MAX_UTT_SEC}
        self._settings_open = True
        try:
            result, vals = SettingsWindow(cur).run_modal()
        except Exception:
            append_err("SettingsWindow: " + traceback.format_exc())
            result, vals = "cancel", {}
        finally:
            self._settings_open = False
        if result == "save" and vals:
            set_config_values(vals)
            apply_settings(self, vals)
            rumps.notification(i18n.t("app_name"), i18n.t("settings_saved_t"), i18n.t("settings_saved_b"))
...
#  与上方 VoiceLogApp(菜单栏路径)完全隔离——纯新增、对菜单栏零影响。复用全部自由函数。
#  这样 SwiftUI App 成为唯一的"脸"(只剩它一个菜单栏图标)，双击即用、自动起停引擎。
# ============================================================================
class HeadlessApp:
    def __init__(self):
    ...
    def run(self):
        ...
        while True:
            time.sleep(2)
            try: self._consume()
            except Exception: append_err("headless consume: " + traceback.format_exc().splitlines()[-1])
            try: self._snapshot()
            except Exception: append_err("headless snapshot: " + traceback.format_exc().splitlines()[-1])

    def _snapshot(self):
        st = self.state
        ts = now()
        snap = {
            "schema": 1, "heartbeat": time.time(), "pid": os.getpid(), "version": VERSION,
            "muted": self.rec.muted, "live": bool(st.get("live")),
            "count": st.get("count", 0), "dropped": st.get("dropped", 0),
            "last": st.get("last", ""), "sink": st.get("sink", "vault"), "err": st.get("err", ""),
            "today_key": f"{ts:%Y-%m-%d}", "today_file": str(VAULT / f"{ts:%Y-%m-%d}.md"),
            "model": {"ready": model_ready(MODEL), "name": Path(str(MODEL)).name,
                      "managed": MANAGED_MODEL, "downloading": bool(st.get("model_dl")),
                      "pct": st.get("model_pct", 0), "dl_id": st.get("model_dl_id"),
                      "result": st.get("model_result"), "result_id": st.get("model_result_id")},
            "speaker": {"enrolled": bool(st.get("enrolled")), "gate_on": self.rec.speaker_on,
                        "last_score": st.get("last_score"), "threshold": SPEAKER_THRESHOLD,
                        "enrolling": bool(st.get("enrolling")),
                        "enroll_progress": st.get("enroll_progress", 0.0),
                        "enroll_voiced": st.get("enroll_voiced", 0.0),
                        "enroll_quality": st.get("enroll_quality")},
            "update": {"checked": bool(st.get("update_checked")), "latest": st.get("update_latest")},
            "paths": {"data_dir": str(DATA), "vault": str(VAULT),
                      "fallback": str(FALLBACK), "config": str(CONFIG_PATH)},
            "settings": _config_snapshot(),
        }
        tmp = STATE_PATH.parent / (STATE_PATH.name + ".tmp")
        tmp.write_text(json.dumps(snap, ensure_ascii=False), encoding="utf-8")
    ...
    def _consume(self):
        ...
        for c in cmds:
            try:
                ...
                append_err(f"headless cmd {c!r}: " + traceback.format_exc().splitlines()[-1])

    def _dispatch(self, cmd, a):
        rec = self.rec
        if cmd in ("pause", "resume", "toggle_mute"):
            rec.muted = {"pause": True, "resume": False}.get(cmd, not rec.muted)
        elif cmd == "speaker_gate":
            rec.speaker_on = bool(a.get("on")); set_config_flag("speaker_gate", rec.speaker_on)
        elif cmd == "enroll":
            rec.enroll()
        elif cmd == "cancel_enroll":
            rec.cancel_enroll()
        elif cmd == "set_vault":
            if a.get("path"): set_vault(a["path"])
        elif cmd == "set_timezone":
            set_timezone(a.get("tz", "") or "")
        elif cmd == "set_language":
            m = {"follow": "", "none": ""}
            if "ui" in a: v = m.get(a["ui"], a["ui"]); set_config_str("ui_language", v); i18n.set_language(v)
            if "primary" in a: v = m.get(a["primary"], a["primary"]); set_config_str("primary_language", v); i18n.set_primary(v)
            if "secondary" in a: v = m.get(a["secondary"], a["secondary"]); set_config_str("secondary_language", v); i18n.set_secondary(v)
        elif cmd == "save_keywords":
            res = parse_corrections(a.get("text", ""))
            if isinstance(res, tuple) and len(res) == 2: write_corrections(res[0], res[1])
        elif cmd == "set_params":
            keys = ("vad_threshold", "min_speech_ms", "min_silence_ms", "min_rms_dbfs",
                    "speaker_threshold", "speaker_min_verify_sec", "max_utterance_sec")
            vals = {k: a[k] for k in keys if k in a}
            if vals: set_config_values(vals); apply_settings(self, vals)
        elif cmd == "open_config":
            ...
            pass   # headless 暂不查更新(由 App 侧负责)
        else:
            append_err(f"headless unknown cmd: {cmd!r}")
...
if __name__ == "__main__":
    ...
    if os.environ.get("VOICELOG_HEADLESS", "").strip().lower() in ("1", "true", "yes"):
        HeadlessApp().run()            # 无菜单栏后台引擎(SwiftUI App 作 sidecar 拉起)
    else:
        VoiceLogApp().run()            # 菜单栏 App(原行为,零改动)
